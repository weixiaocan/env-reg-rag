...
app = create_app(
    ...
    source_lookup_corpus_version=(services.corpus_version if services.corpus_version != 'formal-corpus-v1'
                                  else 'm3-experiment-v1'),
    formula_preview=FormulaPreviewService(PROJECT_ROOT),
    formula_review=FormulaReviewService(PROJECT_ROOT, os.getenv('FORMULA_REVIEW_RUN', '5d3264515515e228')),
    published_regions=PublishedRegionService(PROJECT_ROOT),
)
